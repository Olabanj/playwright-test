...
/**
 * Fluent builder for a Fixed-rate contractor contract's create-time data.
 * No HTTP — pure data construction. `signatoryId`/`templateId` are
 * account-specific and resolved dynamically by `seeding.ts`, not by this
 * builder (see `ContractorContractInput`). Worker name/email reuse the team's
 * uniqueness convention verbatim (root CLAUDE.md "Worker Factory Rules").
 */
export class FixedContractBuilder {
  ...
  withPayCycle(cycle: PayCycle): this {
    ...
    if (!spec) {
      throw new Error(`FixedContractBuilder.withPayCycle: unknown cycle "${cycle}"`);
    }
   ...
   * dedicated `ContractsClient.createCorContract` always sends `is_cor: true`
   * regardless — see `ContractorContractCreateData.isCor` JSDoc.
   */
  asCor(): this {
  ...
  build(): ContractorContractInput {
    const worker = generateWorkerData('fixed-contractor');
    ...
    const defaults: ContractorContractInput = {
      ...
      contractName:       `QA Fixed ${Date.now()}`,
    };
